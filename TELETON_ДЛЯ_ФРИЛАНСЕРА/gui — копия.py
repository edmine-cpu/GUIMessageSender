...
class BroadcastFrame(ctk.CTkFrame):
    ...
    def _mass_start_everything(self):
        ...
        try:
            if hasattr(self, "_cycle_start_enabled_campaigns"):
                ...
                started.append("циклы (все enabled)")
        except Exception as e:
    ...
    def _mass_stop_everything(self):
        ...
        try:
            if hasattr(self, "_stop_cycle"):
                self._stop_cycle()  # без имени = стоп текущего/всех
                stopped.append("циклы")
            # Также пробуем остановить enabled runner если есть
            if hasattr(self, "_cycle_stop_event"):
    ...
    def _cycle_build_snapshot(self) -> dict:
        ...
        return {
            ...
            "last_account": (state.get("last_account_phone") or "").strip() or "—",
            "last_run_at": self._cycle_format_dt(state.get("last_run_at", "")),
            "last_sent_at": last_sent_dt.strftime("%Y-%m-%d %H:%M:%S") if last_sent_dt else "—",
            ...
            "last_error": (state.get("last_error") or "").strip() or "—",
        }
    ...
    def _cycle_watchdog(self):
        ...
        try:
            ...
            selected_alive = self._cycle_runner_alive(self._cycle_get_runner(selected))
        ...
        try:
            ...
            for name, ntgts, naccs in configured:
                ...
                try:
                    ...
                    # Проставляем enabled=True, чтобы мониторинг знал, что она должна жить
                    try:
    ...
    def _cycle_update_status(self):
        ...
        try:
            ...
            summary_bottom = (
                ...
                f"следующая: {snap['next_retry_at']} | ошибка: {self._cycle_short_text(snap['last_error'], 42)}"
            )
    ...
    def _start_cycle(self):
        ...
        def thread():
            ...
            try:
                ...
                _thread_local.log_handler = None
                log_queue.put(("cycle_done", running_campaign_name))
    ...
    def on_queue_message(self, tag, msg):
        if tag == "quick_log":
            ...
            self.btn_stop_current.configure(state="disabled", text="■ Остановить текущий процесс")
            try:
                self.lbl_quick_status.configure(text="Статус: готов", text_color="gray70")
            except Exception:
                pass
        elif tag == "cycle_done":
        ...
        try:
            while processed < max_batch:
                ...
                try:
                    ...
                    elif tag.startswith("broadcast") or tag.startswith("mention") or tag == "check_done":
